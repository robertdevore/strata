...
Apply admission, safety, scope, deduplication, provenance, and retrieval rules. A no-write result is correct when no durable memory delta survives admission; handoffs are milestone-driven, not mandatory. Store admitted agent-created notes in `Agent Notes` through the CLI or documented local API. Never write directly to SQLite and never claim a write succeeded without confirmation.
