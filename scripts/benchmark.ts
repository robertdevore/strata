...
for (const count of sizes) {
  ...
  const insert = fixture.prepare(
    'INSERT INTO notes (id,content,created_at,updated_at,tags,title,normalized_title) VALUES (?,?,?,?,?,?,?)',
  )
  const ids: string[] = []
    ...
    for (let i = 0; i < count; i++) {
      ...
      insert.run(
        ...
        `# Topic ${i}\n\n[[Topic ${Math.max(0, i - 1)}]]\n` +
          `Deterministic knowledge category${i % 100} project decisions and context. `.repeat(30),
        now,
        now,
        JSON.stringify([`category${i % 100}`, 'knowledge']),
        `Topic ${i}`,
        `topic ${i}`,
      )
    }
  ...
  measure('updateLinks', () =>
    db.updateNote(ids[50], { content: `# Topic 50\n[[Topic 49]]\n${randomUUID()}` }),
  )
  measure('backlinks', () => db.getBacklinks(ids[49]))
  measure('related', () => db.getRelatedNotes(ids[49]))
  const payloads = {
    list50: Buffer.byteLength(JSON.stringify(db.listSummaryPage({ limit: 50 }).notes)),
    search10: Buffer.byteLength(JSON.stringify(db.aiSearchNotes('category42', 10))),
  }
