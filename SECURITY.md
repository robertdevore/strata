...
- The renderer is sandboxed with context isolation and no Node integration. The arbitrary `window.strata.shell.run` bridge has been removed. Publishing uses bounded inputs, a canonical native-selected destination and exclusive file creation; a changed directory alias cannot redirect the write.
