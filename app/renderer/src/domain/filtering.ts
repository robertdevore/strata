...
const sortNotes = (notes: Note[], sortMode: SortMode): Note[] => {
  ...
  if ('title_asc' === sortMode)
    return copy.sort((a, b) => deriveNoteTitle(a.content).localeCompare(deriveNoteTitle(b.content)))
...
export const applyFiltersAndSort = (
  notes: Note[],
  view: ViewFilter,
  project_names_by_id: Record<string, string> = {},
): Note[] => {
  const filtered = notes.filter((note) => {
    ...
    const matches_query =
      searchMatch(note, view.searchQuery) ||
      Boolean(
        note.projectId &&
        project_names_by_id[note.projectId]?.toLowerCase().includes(view.searchQuery.toLowerCase()),
      )
