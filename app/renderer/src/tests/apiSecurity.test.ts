...
afterEach(async () => {
  ...
  delete process.env.STRATA_API_CREDENTIAL_FILE
})
const start = async () => {
...
describe('local security boundaries', () => {
  it('rejects unauthenticated, cross-origin, oversized and invalid content requests', async () => {
    ...
    expect(
      (await fetch(`${url}/notes`, { headers: { ...headers, Origin: 'https://hostile.example' } })).status,
    ).toBe(403)
    expect(
      (
  ...
  it('refuses non-loopback addresses', async () => {
    ...
    for (const host of ['0.0.0.0', '::', '192.168.1.2'])
      await expect(startNotesApiServer(db, { host, port: 0 })).rejects.toThrow('Non-loopback')
  })
  it('generates a persistent owner-only credential', () => {
    ...
    expect(readLocalCredential()).toBe(token)
    expect(ensureLocalCredential()).toBe(token)
  ...
  it('blocks renderer navigation and unsupported external URLs', () => {
    ...
    protectNavigation({
      on: (name: string, fn: (typeof events)[string]) => {
        events[name] = fn
      },
