...
describe('cli agent safety guards', () => {
  ...
  it('blocks destructive actions in agent mode without allow flag', () => {
    expect(() =>
      ensure_agent_destructive_allowed({ ...base_options, agentMode: true }, false, 'notes delete'),
    ).toThrow(CliError)
    expect(() =>
      ensure_agent_destructive_allowed({ ...base_options, agentMode: true }, true, 'notes delete'),
    ).not.toThrow()
  })
})
...
describe('cli agent context search', () => {
  ...
  it('returns complete note records only when --full is explicit', async () => {
    const client = {
      searchNotes: vi.fn().mockResolvedValue([{ ...note, content: '', snippet: 'Compact preview' }]),
      getNote: vi.fn().mockResolvedValue(note),
    } as unknown as StrataApiClient
    const program = new Command().exitOverride()
    const write = vi.spyOn(process.stdout, 'write').mockImplementation(() => true)
    register_agent_commands(program, () => ({ options: { ...base_options, outputMode: 'json' }, client }))
