  ...
  if (match && match[1]) {
    ...
    if (note_ids.length > 0) {
      const note_links = note_ids
        .map((note_id) => {
          const label = note_titles_by_id[note_id]
            ? escape_markdown_label(note_titles_by_id[note_id])
            : note_id
...
export function ChatPanel(props: ChatPanelProps) {
  ...
  const filtered_models = useMemo(() => {
    ...
    return modelCatalog.filter(
      (entry) =>
        entry.model.toLowerCase().includes(query) || entry.providerLabel.toLowerCase().includes(query),
    )
  ...
  const apply_wiki_suggestion = useCallback(
    (note_title: string) => {
      const textarea = composeTextareaRef.current
      if (!textarea) return
  ...
  const copy_message_text = useCallback(
    async (message_id: string, message_content: string) => {
      try {
  ...
  const copy_code_block = useCallback(
    async (code_block_id: string, code_content: string) => {
      try {
    ...
    <aside className="preview-panel chat-panel">
      ...
      <div className="chat-panel-top">
        <div className="chat-thread-row">
          ...
          <button className="icon-button" onClick={onCreateThread} title="New chat" aria-label="New chat">
            <MessagePlusIcon />
          </button>
          <button
            className="icon-button"
            onClick={() => setShowDeleteConfirm(true)}
            disabled={!activeThreadId || deleting}
            title="Delete chat"
          ...
          <button
            className={`icon-button ${showSearchModal ? 'chip-active' : ''}`}
            onClick={() => {
              onSearchQueryChange('')
              onClearSearch()
          ...
          <button
            className={`icon-button ${showUsageModal ? 'chip-active' : ''}`}
      ...
      {(errorMessage || dictationError || isTranscribing || isDictating) && (
        <p className="chat-error">
          {errorMessage || dictationError || (isTranscribing ? 'Transcribing audio…' : 'Listening…')}
        </p>
      )}
          ...
          onKeyDown={(event) => {
            if (wiki_suggestions.length > 0) {
              ...
              if ('ArrowUp' === event.key) {
                event.preventDefault()
                setActiveWikiSuggestionIndex(
                  (current) => (current - 1 + wiki_suggestions.length) % wiki_suggestions.length,
                )
                return
              }
              if ('Escape' === event.key) {
        ...
        >
          <div className="palette-card chat-search-modal" onClick={(event) => event.stopPropagation()}>
            <div className="palette-search-row">
              ...
              />
              <button
                className="icon-button palette-search-close-btn"
                onClick={() => {
                  setShowSearchModal(false)
                  onClearSearch()
                }}
            ...
            <div className="palette-results chat-search-modal-results">
              {chat_search_results.length > 0 ? (
                ...
                <p className="palette-empty">
                  {searchQuery.trim()
                    ? 'No matching chats.'
                    : 'No chats yet. Start a new chat to build history.'}
                </p>
              )}
            </div>
          </div>
        ...
        <div className="modal-overlay" onClick={() => setShowUsageModal(false)}>
          <div className="modal-card chat-usage-modal" onClick={(event) => event.stopPropagation()}>
            <div className="chat-usage-modal-header">
              <div>
                <h3>Chat Usage</h3>
                <p className="chat-usage-modal-subtitle">
                  Analytics from routed model logs for this chat thread.
                </p>
              </div>
              <button
                className="icon-button"
            ...
            {chatUsageLoading ? (
              ...
              <>
                <div className="chat-usage-kpis">
                  <article className="chat-usage-kpi-card">
                    ...
                    <strong className="chat-usage-kpi-value">
                      {format_token_count(chatUsageSummary.totalTokens)}
                    </strong>
                  </article>
                  <article className="chat-usage-kpi-card">
                    <span className="chat-usage-kpi-label">Input</span>
                    <strong className="chat-usage-kpi-value">
                      {format_token_count(chatUsageSummary.inputTokens)}
                    </strong>
                  </article>
                  <article className="chat-usage-kpi-card">
                    <span className="chat-usage-kpi-label">Output</span>
                    <strong className="chat-usage-kpi-value">
                      {format_token_count(chatUsageSummary.outputTokens)}
                    </strong>
                  </article>
                  <article className="chat-usage-kpi-card">
                    <span className="chat-usage-kpi-label">Implied Cost</span>
                    <strong className="chat-usage-kpi-value">
                      {format_implied_cost(chatUsageSummary.impliedCostUsd)}
                    </strong>
                  </article>
                </div>
                {chatUsageSummary.providers.length > 0 ? (
                  <div className="chat-usage-provider-list">
                    {chatUsageSummary.providers.map((provider) => (
                      <article
                        key={`${provider.providerId}-${provider.model}`}
                        className="chat-usage-provider-card"
                      >
              ...
              <p className="chat-usage-empty">
                No usage data yet. Send a message in this chat to generate analytics.
              </p>
            )}
          </div>
        </div>
