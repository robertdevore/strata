      ...
      <div className="modal-card palette-card" onClick={(event) => event.stopPropagation()}>
        <div className="palette-search-row">
          ...
          <input
            ...
            autoFocus
          />
          <button className="icon-button palette-search-close-btn" onClick={onClose} aria-label="Close">
            <CloseIcon size={16} />
          </button>
        </div>
        <div className="palette-results">
            ...
            <>
              ...
              {pinned_filtered.map((tag) => {
                ...
                return (
                  <button
                    key={tag}
                    className={`palette-item ${selectedTag === tag ? 'palette-item-active' : ''}`}
                    onClick={() => {
                      onSelectTag(tag)
                      onClose()
                    }}
                  >
                    ...
                    <span className="palette-item-hint">
                      ...
                      <button
                        className="tag-pin-btn pin-active"
                        onClick={(e) => {
                          e.stopPropagation()
                          onUnpinTag(tag)
                        }}
          ...
          <div
            className={`palette-all-tags-controls ${pinned_filtered.length > 0 ? 'palette-all-tags-controls-separated' : ''}`}
          >
            ...
            <div className="palette-sort-row" role="group" aria-label="Tag sorting options">
              <label className="palette-sort-field">
                ...
                <select
                  value={sort_by}
                  onChange={(event) => set_sort_by(event.target.value as 'alphabet' | 'count')}
                >
              ...
              <label className="palette-sort-field">
                ...
                <select
                  value={sort_order}
                  onChange={(event) => set_sort_order(event.target.value as 'asc' | 'desc')}
                >
          ...
          {sorted_tags.map((tag) => (
            <button
              key={tag.name}
              className={`palette-item ${selectedTag === tag.name ? 'palette-item-active' : ''}`}
              onClick={() => {
