...
import type { AiMessage, AiRouteLog, AiSearchResult, AiThreadSummary, Note, Project } from '@shared/types'
...
const CHAT_TOKEN_PRICING_BY_MODEL_PATTERN: Array<{
  match: RegExp
...
const select_usage_logs_for_model = (
  logs: AiRouteLog[],
  thread_id: string,
  ...
  const preferred_model_logs = thread_logs.filter(
    (log) => normalize_model_id(log.model) === normalized_preferred_model,
  )
...
const listNewlineKeymap = Prec.highest(
  keymap.of([
    {
      key: 'Enter',
      run: (view) => {
...
export function EditorPane(props: EditorPaneProps) {
  ...
  const [backlinks, setBacklinks] = useState<
    Array<{ link: import('@shared/types').NoteLink; source: import('@shared/types').Note }>
  >([])
  ...
  const projectSummaries = useMemo(() => {
    return projects.map((project) => {
      const project_notes = allNotes.filter((note) => note.projectId === project.id)
      const sorted_notes = [...project_notes].sort(
  ...
  useEffect(() => {
    ...
    const togglePreview = () =>
      setShowPreview((v) => {
        const n = !v
        if (n) setShowChatPanel(false)
        return n
      })
    const toggleChat = () =>
      setShowChatPanel((v) => {
        const n = !v
        if (n) setShowPreview(false)
        return n
      })
    ...
    return EditorView.inputHandler.of((view, from, to, text, insert) => {
      ...
      if (from === to) {
        ...
        if (
          ...
          selection.empty &&
          text === pending.marker &&
          Math.abs(selection.from - pending.end) <= 1
        ) {
          view.dispatch({
            changes: [
        ...
        if (
          pending &&
          (Date.now() >= pending.expires_at || selection.from !== pending.end || text !== pending.marker)
        ) {
          pending_wrap_marker_ref.current = null
        }
        return false
                        ...
                        {0 === recentNotes.length ? (
                          ...
                          recentNotes.map((project_note) => (
                            <button
                              ...
                              onClick={() => void onOpenNoteFromChat(project_note.id)}
                            >
                              {deriveNoteTitle(project_note.content)}
                            </button>
                          ))
                        )}
                      </div>
  ...
  const wikiLinkToMarkdown = (md: string): string => {
    ...
    const converted = protected_md.replace(
      /(?<!!)\[\[([^\]|#\n]{1,120})(?:#([^\]|\n]{1,80}))?(?:\|([^\]\n]{1,120}))?\]\]/g,
      (_full, target: string, heading: string | undefined, label: string | undefined) => {
    ...
    <section className="editor">
      <header className="editor-header">
        ...
        <div className="editor-actions">
          ...
          <button className="icon-button" onClick={() => onToggleStar(note.id)} title="Toggle Star">
            {note.starred ? <StarFilledIcon /> : <StarOutlineIcon />}
          </button>
          <button className="icon-button" onClick={() => setShowTagsEditor(true)} title="Tags">
            <TagIcon />
          </button>
          <button
            className={`icon-button ${showPreview ? 'chip-active' : ''}`}
            onClick={() => {
          ...
          <button
            className={`icon-button ${showChatPanel ? 'chip-active' : ''}`}
            onClick={() => {
      ...
      <div
        className={`editor-body ${isPreviewResizing ? 'editor-body-resizing' : ''}`}
        ref={editorBodyRef}
          ...
          while ((match = wiki_re.exec(line_text)) !== null) {
            if (col >= match.index && col <= match.index + match[0].length) {
              ...
              void handleWikiLinkClick(
                `#strata-note:${encodeURIComponent(raw_target)}`,
      ...
      >
        ...
        {showSidePanel && (
          <>
            <div
              className="panel-resizer panel-resizer-preview"
              ...
              <article className="preview-panel preview-markdown">
                <ReactMarkdown
                  ...
                  components={{
                    a: ({ href, children, ...props }) => {
                      const is_wiki =
                        href && (href.startsWith('#strata-note:') || href.startsWith('strata-note://'))
                      if (is_wiki) {
                        return (
                          <a
                            href={href}
                    ...
                    blockquote: ({ children, ...props }) => {
                      ...
                      const callout_match = /^\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\]/i.exec(
                        full_text.trim(),
                      )
                      ...
                      if (callout_match) {
                          ...
                          if (node && typeof node === 'object' && 'props' in node) {
                            ...
                            if (el.props.children !== undefined) {
                              ...
                              if (new_children !== el.props.children) {
                                return {
                                  ...el,
                                  props: { ...el.props, children: new_children },
                                } as React.ReactNode
                              }
                            }
                          }
                        ...
                        const body = Children.map(children, (child) => stripMarker(child))?.filter(
                ...
                {backlinks.length > 0 && (
                  <div className="backlinks-section">
                    <button className="backlinks-toggle" onClick={() => setShowBacklinks((v) => !v)}>
                      {showBacklinks ? '▾' : '▸'} {backlinks.length} Backlink
                        ...
                        {backlinks.map((entry) => (
                          <li key={entry.link.id} className="backlink-item">
                            <button
                              ...
                              onClick={(event) =>
                                void onOpenNoteFromChat(entry.source.id, event.metaKey || event.ctrlKey)
                              }
                            >
                ...
                {relatedNotes.length > 0 && (
                  <div className="related-section">
                    <button className="backlinks-toggle" onClick={() => setShowRelatedPreview((v) => !v)}>
                      {showRelatedPreview ? '▾' : '▸'} {relatedNotes.length} Related Note
                        ...
                        {relatedNotes.map((entry) => (
                          <li key={entry.note.id} className="backlink-item">
                            <button
                              className="backlink-title"
                              onClick={(event) =>
                                void onOpenNoteFromChat(entry.note.id, event.metaKey || event.ctrlKey)
                              }
                            >
      ...
      <footer className="editor-footer">
        <span className="editor-footer-left">
          ...
          <button className="icon-button" onClick={() => setShowDocStats(true)} title="Document Statistics">
            <FileDescriptionIcon />
          </button>
        </span>
        <div
          className={`editor-footer-actions ${footerActionsCollapsed ? 'editor-footer-actions-collapsed' : ''}`}
        >
          {!footerActionsCollapsed && (
            <>
              <button
                className={`icon-button ${showFindReplace ? 'chip-active' : ''}`}
                onClick={() => {
                  setFindReplaceStatus('')
                  setShowFindReplace(true)
                }}
              ...
              <button
                ...
                title="Export / Publish Note"
              >
                <UploadIcon />
              </button>
              <button className="icon-button" onClick={onShowRelatedNotes} title="Related Notes">
                <CirclesRelationIcon />
              </button>
              <button className="icon-button" onClick={() => void printNote()} title="Print Note">
                <PrinterIcon />
              </button>
              <button className="icon-button" onClick={openTtsModal} title="Listen to Note">
                <EarIcon />
              </button>
              <button className="icon-button" onClick={() => onToggleArchive(note.id)} title="Toggle Archive">
                <ArchiveIcon />
              </button>
              <button className="icon-button" onClick={() => onDelete(note.id)} title="Delete Note">
                <TrashIcon />
              </button>
              <button
                className="icon-button"
        ...
        <div className="modal-overlay" onClick={() => setShowFindReplace(false)}>
          <div className="modal-card find-replace-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="modal-actions find-replace-actions">
              <button className="ghost-button" onClick={findNextMatch}>
                Find next
              </button>
              <button className="ghost-button" onClick={replaceSelection}>
                Replace
              </button>
              <button className="primary-button" onClick={replaceAllMatches}>
                Replace all
              </button>
            </div>
          </div>
      ...
      <TagsEditor
        key={showTagsEditor ? `tags-editor-open-${note.id}` : `tags-editor-closed-${note.id}`}
        open={showTagsEditor}
        ...
        <div className="modal-overlay" onClick={() => setShowAiHistory(false)}>
          <div className="modal-card related-notes-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="related-notes-body">
              {aiEdits.length === 0 && (
                <p className="related-notes-empty">No AI edits recorded for this note.</p>
              )}
              {aiEdits.map((edit) => (
                <div
                  key={edit.id}
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 11, color: 'var(--text-3)', textTransform: 'uppercase' }}>
                      {edit.action} • {new Date(edit.createdAt).toLocaleString()}
                    </span>
                    {!edit.revertedAt && (
                      <button
                        ...
                        onClick={async () => {
                          ...
                          if (ok) {
                            setAiEdits((prev) =>
                              prev.map((e) =>
                                e.id === edit.id ? { ...e, revertedAt: new Date().toISOString() } : e,
                              ),
                            )
                          }
                        }}
                    ...
                    {edit.revertedAt && (
                      <span style={{ fontSize: 10, color: 'var(--text-3)' }}>Reverted</span>
                    )}
                  </div>
          ...
          <div className="modal-card tts-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="tts-controls">
              ...
              <div className="tts-button-row">
                {!ttsSpeaking ? (
                  <button className="tts-play-button" onClick={startTts}>
                    Play
                  </button>
                ) : (
                  <>
                    <button className="tts-play-button" onClick={pauseTts}>
                      {ttsPaused ? 'Resume' : 'Pause'}
                    </button>
                    <button className="tts-stop-button" onClick={stopTts}>
                      Stop
                    </button>
                  </>
                )}
