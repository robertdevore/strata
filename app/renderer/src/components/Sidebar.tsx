...
import { ChevronDownIcon, ChevronUpIcon, CircleChevronLeftIcon, CircleChevronRightIcon, CirclePlusIcon, CloseIcon, MoonIcon, PinFilledIcon, PinIcon, SettingsIcon, SunIcon, TrashIcon } from './icons'
...
export function Sidebar(props: SidebarProps) {
	...
	const sectionOrderIndex = useMemo(() => {
		return Object.fromEntries(props.sidebarLayout.sectionOrder.map((section_id, index) => [section_id, index])) as Record<SidebarSectionId, number>
							...
							<div className="pinned-tags">
								{filteredPinnedTags.map((tag) => (
									<button
										...
										onDragOver={(e) => { if (!draggedTag) return; e.preventDefault(); e.dataTransfer.dropEffect = 'move' }}
										onDrop={() => {
											if (!draggedTag) return
										...
										>
											{0 === visible_project_notes.length ? (
												<div className="sidebar-section-empty project-empty-state">
													No notes yet in this project. Drag a note here or import a folder of markdown files.
												</div>
											) : (
												visible_project_notes.map((note) => (
													<div
														key={note.id}
													...
													>
														<div className="note-row-title-wrap">
															...
															>
																{pinnedNoteIdSet.has(note.id) ? <PinFilledIcon size={14} /> : <PinIcon size={14} />}
															</button>
																<button
																	type="button"
									...
									<div className="pinned-notes-empty">
										{hasSidebarSearch ? (
											<>
												...
												<p className="pinned-notes-empty-copy">Try a different phrase or clear the search to see every pinned note again.</p>
											</>
										) : (
											<>
												<p className="pinned-notes-empty-title">No pinned notes yet</p>
												<p className="pinned-notes-empty-copy">Drag a note here or pin the one you’re reading right now.</p>
												<button
									...
									filteredPinnedNotes.map((entry) => (
										...
										>
											<div className="note-row-title-wrap pinned-note-title-wrap">
												...
												<span className="note-row-time">{formatRelativeTime(entry.note?.updatedAt ?? '')}</span>
												<div className="note-row-icons">
													<button
														type="button"
			...
			{!props.sidebarCollapsed && props.undoDeleteTitle && (
				...
				{projectCreateOpen && (
					<div className="modal-overlay" onClick={closeProjectCreateModal}>
						<div className="modal-card sidebar-project-create-modal" onClick={(event) => event.stopPropagation()}>
							<button className="icon-button modal-close-button" onClick={closeProjectCreateModal} aria-label="Close new project" title="Close new project">
								<CloseIcon />
							</button>
							<h3>New Project</h3>
							...
							<form className="sidebar-project-create-form" onSubmit={(event) => void createProject(event)}>
								...
								<div className="modal-actions">
									<button className="ghost-button" type="button" onClick={closeProjectCreateModal} disabled={isProjectCreateBusy}>Cancel</button>
				...
				{menu && (
					...
					<button onClick={() => { props.onSelect(menu.noteId); closeMenu() }}>Open Note</button>
					<div
						...
						{'project' === menuSubmenu && (
							<div className="context-submenu">
								{props.projects.length > 0 ? (
									<>
										<button onClick={() => { props.onSetNoteProject(menu.noteId, null); closeMenu() }}>No Project</button>
										{props.projects.map((project) => (
											<button
												key={project.id}
												onClick={() => {
					...
					<button onClick={() => { props.onStarToggle(menu.noteId); closeMenu() }}>Star / Unstar</button>
					<button onClick={() => { props.onArchiveToggle(menu.noteId); closeMenu() }}>Archive / Unarchive</button>
					<button className="danger" onClick={() => { props.onDelete(menu.noteId); closeMenu() }}>Delete</button>
				</div>
			)}
		</aside>
	)
