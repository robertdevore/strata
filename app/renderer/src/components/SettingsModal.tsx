...
import { get_home_tile_option, HOME_TILE_OPTIONS, normalize_home_tiles, type HomeTileAction } from '@shared/homeTiles'
import { DEFAULT_SIDEBAR_LAYOUT, normalize_sidebar_layout, type SidebarLayoutSettings, type SidebarSectionId } from '@shared/sidebarLayout'
...
export function SettingsModal({
	...
	const [sidebar_layout_draft, set_sidebar_layout_draft] = useState<SidebarLayoutSettings>(DEFAULT_SIDEBAR_LAYOUT)
	...
	const ordered_projects = project_order_draft.length > 0
		? project_order_draft.map((project_id) => projects.find((project) => project.id === project_id)).filter((project): project is Project => Boolean(project))
		: projects
	...
	const update_home_tile = (index: number, action: HomeTileAction) => {
		onUpdate({
			homeTiles: resolved_home_tiles.map((tile, tile_index) => (
				tile_index === index ? { action } : tile
			)),
		})
	}
	...
	const reorder_sidebar_section = (target: SidebarSectionId) => {
		...
		const current_order = sidebar_layout_draft.sectionOrder.filter((section) => section !== dragged_sidebar_section)
	...
	const reorder_projects = async (target_project_id: string) => {
		...
		const current_order = (project_order_draft.length > 0 ? project_order_draft : projects.map((project) => project.id)).filter((project_id) => projects.some((project) => project.id === project_id))
	...
	const delete_project = async (project: Project) => {
		const confirmed = window.confirm(`Delete project "${project.name}"? Notes will stay in Strata but will be unassigned from the project.`)
		if (!confirmed) return
		set_is_project_action_busy(true)
		set_project_action_status('')
		try {
	...
	return (
		<div className="modal-overlay" onClick={onClose}>
			<div className={`modal-card settings-modal-card ${'home' === tab ? 'settings-modal-card-home' : ''} ${'projects' === tab ? 'settings-modal-card-projects' : ''} ${'sidebar' === tab ? 'settings-modal-card-sidebar' : ''}`} onClick={(event) => event.stopPropagation()}>
				...
				<div className="settings-tab-content">
					...
					{'home' === tab && (
						<>
							<p className="tags-label" style={{ marginBottom: 10, lineHeight: 1.4 }}>
								These cards appear when the editor is empty. Pick the three actions that should greet new sessions.
							</p>
							...
							<div className="sidebar-settings-panel">
								<label className="sidebar-settings-toggle-row">
									<span className="sidebar-settings-label-wrap">
										...
										<span className="sidebar-settings-description">Show the search field at the top of the sidebar.</span>
									</span>
									<input
										type="checkbox"
										checked={sidebar_layout_draft.showSearch}
										onChange={(event) => update_sidebar_layout({
											...sidebar_layout_draft,
					...
					{'projects' === tab && (
						<>
							<p className="tags-label" style={{ marginBottom: 10, lineHeight: 1.4 }}>
								Projects group related notes. Rename them here, or create new ones before importing folders or assigning notes in the sidebar.
							</p>
							<form className="project-create-form" onSubmit={(event) => void create_project(event)}>
								...
								<button className="primary-button project-create-button" type="submit" disabled={is_project_action_busy}>
								...
								<div className="project-settings-list">
									{0 === ordered_projects.length ? (
										<div className="sidebar-section-empty project-settings-empty">
											No projects yet. Create one above, or import a folder of markdown files to start grouping notes.
										</div>
									) : (
										ordered_projects.map((project) => {
											...
											return (
												...
												>
													...
													<label className="project-settings-name-field">
														...
														<input
															type="text"
														value={draft_name}
														onChange={(event) => set_project_name_drafts((current) => ({ ...current, [project.id]: event.target.value }))}
														spellCheck={false}
													/>
												</label>
												...
												<div className="project-settings-actions">
													<button className="ghost-button project-settings-save" type="submit" disabled={!can_save || is_project_action_busy}>
														Save
													</button>
													<button
