...
export const SECRET_KEYS = ['openAiApiKey','aiDeepseekApiKey','aiKimiApiKey','aiOpenrouterApiKey','aiCustomApiKey'] as const
...
export class EncryptedSecretStore implements SecretStore {
 ...
 constructor(directory: string, backend: EncryptionBackend) { this.directory = directory; this.backend = backend }
 private check(): void {
  if (!this.backend.isEncryptionAvailable() || this.backend.getSelectedStorageBackend?.() === 'basic_text') throw new Error('OS credential encryption is unavailable; configure an OS keyring before using provider keys')
 }
 get(key: SecretKey): string {
