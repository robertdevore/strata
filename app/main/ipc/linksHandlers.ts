import { handleTrustedIpc } from '../security/trustedIpc'
import { z } from 'zod'
import type { StrataDatabase } from '../db'
import { KnowledgeService } from '../services/knowledgeService'
import { IPC_CHANNELS } from '../../shared/ipc'
const idSchema = z.object({ id: z.string().uuid() })
export const registerLinksHandlers = (db: StrataDatabase, onChanged?: () => void) => {
  const service = new KnowledgeService(db, onChanged)
  handleTrustedIpc(IPC_CHANNELS.linksBacklinks, (_event, payload) =>
    db.getBacklinks(idSchema.parse(payload).id),
  )
  handleTrustedIpc(IPC_CHANNELS.linksResolveTarget, (_event, payload) =>
    service.resolveLinkedNote(z.object({ rawTarget: z.string() }).parse(payload).rawTarget),
  )
  handleTrustedIpc(IPC_CHANNELS.linksCreateMissingNote, (_event, payload) => {
    const { title } = z.object({ title: z.string() }).parse(payload)
    return service.createMissingLinkedNote(title)
  })
  handleTrustedIpc(IPC_CHANNELS.linksRelatedNotes, (_event, payload) =>
    db.getRelatedNotes(idSchema.parse(payload).id),
  )
}
