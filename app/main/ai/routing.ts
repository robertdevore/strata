...
const KEYPHRASE_RULES: Array<{ keyphrases: string[]; rule: KeyphraseRule }> = [
  ...
  // CHEAP: tagging
  {
    keyphrases: ['tag', 'add tags', 'categorize', 'label', 'classify notes', 'organize notes'],
    rule: {
      intent: 'tag_note',
      risk: 'low',
      route: 'cheap',
  ...
  // CHEAP: summarization
  {
    keyphrases: ['summarize', 'sum up', 'give me a summary', 'tldr', 'what does this note say', 'recap'],
    rule: {
  ...
  {
    keyphrases: ['rewrite this search', 'improve this query', 'better search', 'search query'],
    rule: {
