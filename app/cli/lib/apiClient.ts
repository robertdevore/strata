...
export class StrataApiClient {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new CliError({
            ...
            code:
              typeof (error_body as { error?: { code?: unknown } })?.error?.code === 'string'
                ? (error_body as { error: { code: string } }).error.code
                : 'STRATA_API_ERROR',
            hint:
              401 === response.status
  ...
  ): Promise<z.infer<typeof note_list_response_schema>> {
  ...
  async captureMemory(
    payload: { content: string; tags: string[]; projectId?: string; projectName?: string },
    dedupe = true,
    dryRun = false,
  ) {
    return this.request<{ note: z.infer<typeof note_schema>; duplicate: boolean; dryRun: boolean }>(
      'POST',
      '/memory/capture',
      {
        body: { payload, dedupe, dryRun },
        validate: z.object({ note: note_schema, duplicate: z.boolean(), dryRun: z.boolean() }),
      },
    )
  }
  async updateNote(
  ...
  async updateProject(id: string, payload: { name: string }): Promise<z.infer<typeof project_schema> | null> {
    const response = await this.request<{ project: z.infer<typeof project_schema> | null }>(
      'PATCH',
      `/projects/${id}`,
      {
        body: payload,
  ...
  async reorderProjects(project_ids: string[]): Promise<Array<z.infer<typeof project_schema>>> {
    const response = await this.request<z.infer<typeof project_list_response_schema>>(
      'POST',
      '/projects/reorder',
      {
