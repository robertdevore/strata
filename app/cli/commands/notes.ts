...
export const register_notes_commands = (
        ...
        : command_options.project
          ? projects.find(
              (project) =>
                ...
                normalize_project_name(command_options.project ?? '').toLowerCase(),
            )?.id
          : undefined
  ...
  notes
    ...
    .action(async function (
      ...
      const content_from_source =
        command_options.content || command_options.file || command_options.stdin
      ...
      const payload = note_update_patch_schema.parse({
        expectedRevision: command_options.ifRevision
          ? Number(command_options.ifRevision)
        ...
        archived: parse_optional_boolean(command_options.archived),
        projectId:
          command_options.clearProject || command_options.project || command_options.projectId
            ? next_project_id
            : undefined,
      })
  ...
  notes
    ...
    .action(async function (command_options: {
      ...
      if (!options.dryRun) {
        ensure_agent_destructive_allowed(
          options,
          Boolean(command_options.allowDestructive),
          'notes delete-many',
        )
      }
  ...
  const archive_action = async (command: Command, note_id: string, archived: boolean): Promise<void> => {
