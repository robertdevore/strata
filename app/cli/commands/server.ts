...
export const resolve_server_bind_environment = (
  base_url: string,
): { STRATA_API_HOST: string; STRATA_API_PORT: string } => {
  const parsed = new URL(base_url)
  if ('http:' !== parsed.protocol) throw new Error('The standalone server requires an http:// base URL.')
  if (parsed.pathname && '/' !== parsed.pathname)
    throw new Error('The standalone server base URL cannot include a path.')
  return {
