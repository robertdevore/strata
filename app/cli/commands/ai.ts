...
const build_route_decision = (message: string) => {
  const mode = (process.env.STRATA_AI_ROUTING_MODE || 'auto') as
    | 'auto'
    | 'cheap_only'
    | 'premium_only'
    | 'ask_each_time'
...
const build_ai_note_payload = (
  raw_input: string,
): { content: string; tags: string[]; route: ReturnType<typeof build_route_decision> } => {
