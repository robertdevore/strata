...
Remaining audit observation: the OpenAI environment-key helper is present, but the route settings map appears to use only database/vault values. Verify and fix environment-key routing during the remaining provider compatibility audit. Project UI counts still need to use complete SQL counts; backup-restore draft preservation and final performance/release deliverables remain in the broader audit.
