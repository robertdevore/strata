...
### Desktop restore and safety-backup recovery verified

The real Electron verification now creates a backup, types another draft immediately before invoking restore, restarts into the selected backup, then restores the generated pre-restore safety backup and verifies the latest draft survived. Both restores/restarts passed in a disposable library, alongside all prior editor/history/concurrency/AI/security scenarios. The harness explicitly owns relaunch and waits for the rendered UI before issuing the next restore. Early harness runs exposed a pending-navigation wait after intentional navigation denial, an incorrect test filter field, Markdown whitespace expectations and a preload-before-UI readiness race; those fixture issues were corrected without weakening the product assertions. Log: `/tmp/strata-restore-desktop.log`.

A fresh dependency audit now reports four affected packages (one high, two moderate, one low): js-yaml, Vitest/@vitest/mocker and joi. Earlier zero-advisory results are historical. Dependency-path verification and compatible fixes are the next work item before remote verification. GitHub's open-issues query still returned an empty list. Origin/main remains an ancestor of the local changes.

### Patch newly reported development dependencies

Updated Vitest/@vitest packages from 3.2.6 to 4.1.11, js-yaml from 4.3.1 to 4.3.2 and joi from 18.2.3 to 18.2.8. Verified package paths and upstream advisories; all affected paths are development/build tooling. Removed Vitest's obsolete minWorkers setting while retaining maxWorkers=2. No application dependency version was changed. Detailed rationale and primary advisory links: `docs/hardening/dependencies.md`.

`npm run verify` passed **257 tests / 60 files**, formatting, lint, TypeScript and production build under Vitest 4.1.11. Log: `/tmp/strata-patched-dependencies-verify.log`. The install audit reported zero vulnerabilities; a separate JSON audit was also run. This supersedes the newly discovered four-package advisory checkpoint, not the historical evidence itself.

### Installed CLI documentation and diagnostics

The npm archive now includes its canonical CLI guide and linked API/security guides. CLI documentation describes installation outside the checkout, credential discovery, ranked FTS summaries/cursors, revision-aware updates, history and batch requests, and confirmed imports; unsupported speculative provider variables and stale substring-only descriptions were removed. `config doctor` now checks the actual API/authentication context without requiring a repository/package.json or legacy npm scripts, and no longer calls missing credentials valid for an unauthenticated API. Search help describes the implemented ranked FTS/fallback behavior.

`npm run package:verify` passed a fresh production tarball installation, native SQLite, authenticated standalone server, `config doctor --fail-on-warning` outside the checkout, existence of installed guides, capture deduplication, retrieval and shutdown. TypeScript and scoped ESLint passed. Log: `/tmp/strata-installed-docs-verify.log`.

Checkpoint `bbfe9fa` and all preceding goal commits were pushed to origin/main. [GitHub Verify run 34290676029](https://github.com/robertdevore/strata/actions/runs/34290676029) passed its Ubuntu verification/audit/installed-package job; macOS source verification/audit/installed-package steps passed and packaging/desktop checks were still running at this observation. GitHub's fresh open Dependabot-alert query returned no entries after the updated lockfile was pushed; the push-time warning referred to its prior default-branch analysis.

### Preserve CLI machine errors and validate origins safely

Reproduced `strata --json notes get invalid-note-id` printing plain text rather than JSON. The entrypoint's failure path now resolves output flags independently of connection configuration, so an invalid environment URL cannot throw again while formatting the original error. Explicit agent mode also wins over an environment pretty-output default. Commander parse failures consistently use validation exit status. Both success and error machine responses now use one JSON line, as required by the source specification.

CLI URL normalization now validates HTTPS origins (or HTTP loopback) and rejects credentials, paths, queries and fragments instead of silently discarding them or exposing invalid URL input in an error. Config and transport use the same normalization, including IPv6 loopback recognition.

Verification: the full CLI directory passed 33 tests before the final compact-output assertion; the subsequent 11 entrypoint/config tests passed, covering malformed note IDs, unknown/missing commands/options, invalid environment URLs, credential-bearing URLs, insecure LAN HTTP, and compact successful output. TypeScript and scoped ESLint/format checks passed. Final whole-suite verification will include the resulting 265-test inventory. GitHub Verify run 34290676029 completed successfully on both Linux and macOS, including the macOS native packaging and desktop restore checks, for checkpoint bbfe9fa.

### Bound CLI responses and preserve transport error semantics
